				       ...
				       struct thread *th, pid_t pid)
{
	...
	if (!leader->mg)
		thread__set_map_groups(leader, map_groups__new(), 0);
			    ...
			    u64 ip, u64 timestamp)
{
	 ...
	 */
	thread__find_cpumode_addr_location_time(thread, machine, MAP__FUNCTION,
						ip, &al, timestamp);
			     ...
			     u64 timestamp)
{
	...
	thread__find_addr_location_time(thread, machine, m, MAP__VARIABLE, addr,
					&al, timestamp);
	if (al.map == NULL) {
		/*
		 ...
		 */
		thread__find_addr_location_time(thread, machine, m, MAP__FUNCTION,
						addr, &al, timestamp);
	}
				     ...
				     struct addr_location *al)
{
	...
	ip__resolve_ams(al->machine, al->thread, &mi->iaddr,
			sample->ip, sample->time);
	ip__resolve_data(al->machine, al->thread, al->cpumode,
			 &mi->daddr, sample->addr, sample->time);
					   ...
					   struct addr_location *al)
{
	...
	for (i = 0; i < bs->nr; i++) {
		ip__resolve_ams(al->machine, al->thread,
				&bi[i].to, bs->entries[i].to, sample->time);
		ip__resolve_ams(al->machine, al->thread,
				&bi[i].from, bs->entries[i].from, sample->time);
