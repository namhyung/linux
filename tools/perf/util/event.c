...
static void map_groups__find_addr_map(struct map_groups *mg,
...
{
	al->thread = thread;
	map_groups__find_addr_map(thread->mg, machine, cpumode, type, addr, al);
}

void thread__find_addr_map_time(struct thread *thread,
				struct machine *machine, u8 cpumode,
				enum map_type type, u64 addr,
				struct addr_location *al, u64 timestamp)
{
	...
	al->thread = thread;
	map_groups__find_addr_map(mg, machine, cpumode, type, addr, al);
}
...
void thread__find_addr_location_time(struct thread *thread,
				     struct machine *machine, u8 cpumode,
				     enum map_type type, u64 addr,
				     struct addr_location *al, u64 timestamp)
{
				  ...
				  struct perf_sample *sample)
{
	 ...
	 */
	thread__find_addr_map_time(thread, machine, cpumode, MAP__FUNCTION,
